...
// Toggle the counter bit-width by uncommenting ONE of the pairs below.
// The plaintext item ID and baseline will remain 32-bit.
// ============================================================================

// --- 8-Bit Counters ---
type FheCount = FheUint8;
type ClearCount = u8;

// --- 16-Bit Counters ---
// type FheCount = FheUint16;
// type ClearCount = u16;

// --- 32-Bit Counters ---
// type FheCount = FheUint32;
// type ClearCount = u32;
...
struct EncryptedBucket {
    id: FheUint32,
    count: FheCount, // Uses the type alias defined above
}

struct EncryptedState {
    buckets: Vec<EncryptedBucket>,
    min_val: FheCount,
    constants: (FheCount, FheCount), // (zero, one)
}
...
// Helper function for O(N) depth sequential first-one evaluation
fn sequential_first_one(cands: &[FheBool]) -> (Vec<FheBool>, FheBool) {
    let len = cands.len();
    if len == 0 {
        return (Vec::new(), FheBool::encrypt_trivial(false));
    }

    let mut out_mask = Vec::with_capacity(len);
    let mut found_any = FheBool::encrypt_trivial(false);

    for cand in cands {
        // A candidate is only valid if it is a match AND we haven't found a match yet
        let not_found_yet = !&found_any;
        let masked_cand = cand & &not_found_yet;
        out_mask.push(masked_cand);

        // Update the running flag: true if previously true, OR if the current candidate is true
        found_any = &found_any | cand;
    }
...
fn process_packet_oblivious(state: &mut EncryptedState, item: &FheUint32, profiler: &mut StepProfiler) {
    ...
    state.buckets.par_iter_mut()
        ...
        .for_each(|(i, bucket)| {
            ...
            // Update Count (Add 0 or 1) - Automatically scales to 8/16/32 bit
            bucket.count = &bucket.count + &increment;
            
            // Update ID (Must use Select/Mux) - Stays 32-bit
...
fn main() {
    ...
    println!("Counter Bit-Width: {} bits", std::mem::size_of::<ClearCount>() * 8);
    println!("---------------------");
    ...
    // Initialize Encrypted State with the dynamic FheCount type
    let mut encrypted_state = EncryptedState {
        buckets: (0..table_size).map(|_| EncryptedBucket {
            id: FheUint32::encrypt(0_u32, &client_key),
            count: FheCount::encrypt(0 as ClearCount, &client_key),
        }).collect(),
        min_val: FheCount::encrypt(0 as ClearCount, &client_key),
        constants: (
            FheCount::encrypt_trivial(0 as ClearCount), // Zero
            FheCount::encrypt_trivial(1 as ClearCount)  // One
        ),
    };
    ...
    for (_i, bucket) in encrypted_state.buckets.iter().enumerate() {
        ...
        // Decrypt into the target type, then cast to u32 for clean printing
        let count: ClearCount = bucket.count.decrypt(&client_key);
        println!("{:<10} | {:<10}", id, count as u32);
    }

    let final_min: ClearCount = encrypted_state.min_val.decrypt(&client_key);
    println!("Final Min Floor: {}", final_min as u32);

    let _reference_table = run_plaintext_simulation(&stream_data, table_size); 
}

// Plaintext reference function remains unchanged and uses 32-bit types
